<template>
    <div
      :class="[
        ...
        props.size === 'large' ? 'flex-row items-stretch' : 'flex-col',
        props.border ? 'border border-gray-default relative overflow-hidden' : '',
      ]"
    >
      <div :class="props.size === 'large' ? 'w-1/2 flex-shrink-0' : 'w-full'">
        <div
          class="inset-0 flex"
          :class="[
            props.size === 'large' ? 'min-h-[26rem] items-start justify-center' : 'items-start justify-center'
          ]"
        >
          <div class="flex-1 flex flex-col w-full" :class="props.size === 'large' ? 'justify-start h-full' : ''">
            <template v-if="props.useStaticImage && props.imageSrc">
              <NuxtImg
                :src="props.imageSrc"
                :alt="props.imageAlt || ''"
                :srcset="props.imageSrc + ', ' + props.imageSrc.replace('.png', '@2x.png') + ' 2x'"
                :class="props.size === 'large' ? 'w-full h-full sm:-mb-16 self-start' : 'w-full object-contain object-top'"
              />
            </template>
          </div>
        </div>
      ...
      <div  
        :class="[
          'bg-white',
          props.size === 'large' ? 'p-16 w-1/2 flex-shrink-0' : 'p-4 w-full'
        ]"
      >
        <div class="flex flex-col justify-between h-full">
          <div class="flex flex-col gap-y-1">
            <p v-if="props.theme" class="uppercase tracking-wider theme-card">{{  props.theme }}</p>
            <p v-if="props.size === 'large'" class="text-4xl font-bold" v-html="props.title"></p>
            <p v-if="props.description" class="text-gray-dark" :class="props.size === 'large' ? 'text-left text-xl' : 'text-base'">
              {{ props.description }}
            </p>
            <p v-if="props.tagline" class="font-semibold text-gray-dark">
              {{ props.tagline }}
            </p>
            <div v-if="props.site || props.code || props.contact" class="product-links mb-4">
              <a 
                v-if="props.site" 
                :href="props.site" 
                target="_blank" 
              ...
              >
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path fill-rule="evenodd" clip-rule="evenodd" d="M1.99992 2H13.9999C14.3681 2 14.6666 2.29848 14.6666 2.66667V13.3333C14.6666 13.7015 14.3681 14 13.9999 14H1.99992C1.63173 14 1.33325 13.7015 1.33325 13.3333V2.66667C1.33325 2.29848 1.63173 2 1.99992 2ZM13.3333 4.82533L8.04792 9.55867L2.66659 4.81067V12.6667H13.3333V4.82533ZM3.00725 3.33333L8.04059 7.77467L13.0013 3.33333H3.00725Z" fill="#3558A2"/>
                </svg>
                Contact
              </a>
            </div>
          </div>
          <div v-if="props.ctaUrl" class="mt-auto text-right">
            <a
              :href="props.ctaUrl"
            ...
            >
              ...
              >
                <path d="M10.9403 12.8572L7.58776 9.40909L8.47163 8.5L13.333 13.5L8.47163 18.5L7.58776 17.5909L10.9403 14.1429H3.33301V12.8572H10.9403Z" fill="#3558A2"/>
              </svg>
            </a>
          </div>
        </div>
