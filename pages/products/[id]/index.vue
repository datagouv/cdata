<template>
  <div class="fr-container">
    <Breadcrumb>
      <BreadcrumbItem :external="true" to="/">
        {{ t('Accueil') }}
      </BreadcrumbItem>
      <BreadcrumbItem :external="true" to="/products">
        {{ t('Produits') }}
      </BreadcrumbItem>
        ...
        <TabGroup size="md" @change="switchTab">
          ...
          <TabPanels>
            <TabPanel v-for="tab in tabsOptions" :key="tab.key">
              <div v-if="tab.key === 'equipe'">
                <h3 class="fr-h4">Notre équipe</h3>
                <p class="fr-text--lg">
                  Notre équipe dédiée travaille quotidiennement pour développer et maintenir 
                  cette infrastructure de données publiques. Composée d'experts en données, 
                  de développeurs et de spécialistes de l'administration publique, nous 
                  nous engageons à fournir des solutions robustes et durables.
                </p>
                <div v-if="teamForProduct.length > 0" class="fr-mt-6v">
                  <div class="team-grid">
                    <div 
              ...
              <div v-if="tab.key === 'roadmap' && card.roadmap_url" class="fr-mt-4v">
                <h3 class="fr-h4">Feuille de route</h3>
                <p class="fr-text--lg">
              ...
              <div v-if="tab.key === 'budget' && card.budget_url" class="fr-mt-4v">
                <h3 class="fr-h4">Budget et financement</h3>
                <p class="fr-text--lg">
                ...
                <div class="fr-mt-6v">
                  <GristTableViewer 
                    ...
                    :unitColumn="card.budget_column_total"
                  />
                </div>
              </div>
            </TabPanel>
...
onMounted(async () => {
  ...
  team.value = dataTeam.records.map((r: any) => r.fields)
  loading.value = false
})
...
const teamForProduct = computed(() => {
  ...
  return team.value.filter((member: any) => member.product === card.value.slug)
})
