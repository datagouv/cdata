...
export default defineEventHandler(async (event) => {
  try {
    ...
    const messages = [
      ...
      {
        role: 'user',
        content: `Extrais entre ${minTagsNb} et ${maxTagsNb} mots-clés pertinents à partir de cette description de jeu de données. N'intègre pas les mots-clés "données", "data", "données ouvertes", "transparence" ou "open data", car ils sont trop généraux. Ne mets pas de majuscule au mot clé sauf si c'est un acronyme. Retourne uniquement la liste séparée par des virgules :\n\n${description}`
      }
    ]
