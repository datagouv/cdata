<template>
    <Listbox v-slot="{ activeOption }" v-model="model">
      <div class="relative">
        <div ref="floatingReference"
        ...
        <ListboxOptions ref="popover" :style="floatingStyles"
            ...
            <ListboxOption v-for="option in options" :key="getOptionId(toValue(option))" v-slot="{selected}"
                           as="template" :value="option">
                <li class="relative cursor-default select-none py-2 pr-4 list-none flex items-center gap-2 text-gray-900"
                    :class="{
                    ...
                    <div class="flex items-center justify-center aspect-square">
                        <RiCheckLine v-if="selected" class="size-4 text-new-primary" />
                    </div>
                    <slot
                        name="option"
