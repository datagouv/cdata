...
// FIXME(charts): the save flow POSTs to /api/1/visualizations/{id}/image/ which udata answers
// with 405 METHOD NOT ALLOWED. The failed save never triggers the refresh GET this test awaits,
// so it hangs until the 600s per-test timeout and blows the 30min CI budget. Skipped until the
// chart image upload is fixed (owner: Nicolas).
test.skip('saving chart sends correct data to API', async ({ page, baseURL }) => {
  await page.goto('/design/chart')
  await page.waitForLoadState('networkidle')
...
// FIXME(charts): same as above — the /api/1/visualizations/{id}/image/ POST returns 405, the save
// flow aborts and this test hangs on the awaited refresh GET until the 600s timeout. Skipped until
// the chart image upload is fixed (owner: Nicolas).
test.skip('complete chart configuration flow', async ({ page, baseURL }) => {
  await page.goto('/design/chart')
  await page.waitForLoadState('networkidle')
