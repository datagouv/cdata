...
test.describe('Password reset', () => {
  ...
  test('shows an error with an invalid token', async ({ page }) => {
    ...
    await page.getByLabel('Confirmer le mot de passe').fill('@1337NewPassword42')
    await page.getByRole('button', { name: 'Réinitialiser le mot de passe' }).click()

    // The reset must fail: error displayed, no success toast, still on the token page
    await expect(page.locator('.fr-error-text, .fr-banner--error, [class*="danger"]').first()).toBeVisible()
